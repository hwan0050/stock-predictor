# ✅ Stock Predictor - 개발 체크리스트

> **현재 버전**: v0.9.2  
> **진행률**: 90%  
> **마지막 업데이트**: 2025-11-16

---

## 📊 전체 진행 현황

```
██████████████████░░  90%
```

| Phase | 작업 내용 | 진행률 | 상태 |
|-------|----------|--------|------|
| Phase 1 | 기본 구축 | 100% | ✅ 완료 |
| Phase 2 | UI/UX 개선 | 100% | ✅ 완료 |
| Phase 3 | 문서화 | 100% | ✅ 완료 |
| Phase 4 | 추가 기능 | 100% | ✅ 완료 |
| Phase 5 | 차트 기능 확장 | 100% | ✅ 완료 |
| Phase 6 | 비교 & 분석 | 0% | 📅 예정 |
| Phase 7 | 배포 준비 | 0% | 📅 예정 |

---

## Phase 1: 기본 구축 ✅

**목표**: Spring Boot + React 기본 앱 구축  
**진행률**: 100% ████████████████████

### Backend (Spring Boot)
- [x] Spring Boot 프로젝트 생성
- [x] Gradle 설정
- [x] Yahoo Finance API 연동
- [x] StockController 구현
- [x] YahooFinanceService 구현
- [x] StockDataDto 구현
- [x] CORS 설정
- [x] API 테스트

### Frontend (React)
- [x] React 프로젝트 생성 (CRA)
- [x] 기본 컴포넌트 구조
- [x] Axios API 연동
- [x] Chart.js 설정
- [x] 환경변수 설정 (.env)
- [x] 기본 라우팅

### API 엔드포인트
- [x] `GET /api/stocks/{symbol}` - 현재 주가 조회
- [x] `GET /api/stocks/{symbol}/history?days=30` - 과거 데이터 조회

---

## Phase 2: UI/UX 개선 ✅

**목표**: 사용자 경험 향상  
**진행률**: 100% ████████████████████

### 디자인 개선
- [x] 반응형 디자인 (Mobile First)
- [x] CSS Variables 시스템
- [x] 그리드 레이아웃
- [x] 카드 디자인
- [x] 그라데이션 효과

### 다크모드
- [x] ThemeToggle 컴포넌트
- [x] LocalStorage 테마 저장
- [x] CSS Variables 활용
- [x] 부드러운 전환 애니메이션

### 애니메이션
- [x] 페이드인 효과
- [x] 슬라이드 업 효과
- [x] 호버 효과
- [x] 차트 애니메이션

### 컴포넌트
- [x] SearchBar 컴포넌트
- [x] StockCard 컴포넌트
- [x] StockChart 컴포넌트
- [x] SearchHistory 컴포넌트
- [x] ThemeToggle 컴포넌트

---

## Phase 3: 문서화 ✅

**목표**: 프로젝트 문서 작성  
**진행률**: 100% ████████████████████

### 문서 파일
- [x] README.md (메인)
- [x] frontend/README.md
- [x] backend/README.md
- [x] CONTRIBUTING.md
- [x] LICENSE (MIT)
- [x] CHECKLIST.md (이 파일)

### README 구성
- [x] 프로젝트 소개
- [x] 기능 설명
- [x] 기술 스택
- [x] 설치 방법
- [x] 사용 방법
- [x] API 문서
- [x] 기여 방법
- [x] 라이센스

### 코드 문서화
- [x] 주석 작성
- [x] JSDoc (일부)
- [x] JavaDoc (일부)
- [x] 변수명 명확화

---

## Phase 4: 추가 기능 ✅

**목표**: 사용성 향상 기능 추가  
**진행률**: 100% ████████████████████

### 1. 404 페이지
- [x] NotFound 컴포넌트 구현
- [x] 404 페이지 디자인
- [x] React Router 설정
- [x] 홈으로 돌아가기 버튼
- [x] 다크모드 지원

### 2. 로딩 애니메이션
- [x] LoadingSpinner 컴포넌트
    - [x] 이중 회전 애니메이션
    - [x] 튀는 점 애니메이션
    - [x] 다크모드 지원
- [x] SkeletonCard 컴포넌트
    - [x] Shimmer 효과
    - [x] 카드 레이아웃
    - [x] 다크모드 지원
- [x] SkeletonChart 컴포넌트
    - [x] 막대 애니메이션
    - [x] 차트 레이아웃
    - [x] 다크모드 지원
- [x] 300ms 타이머 최적화
- [x] clearTimeout 메모리 관리

### 3. 검색 기능 개선
- [x] 자동완성 기능
    - [x] 50개 주요 종목 데이터
    - [x] 필터링 로직
    - [x] 키보드 네비게이션 (↑↓ Enter)
    - [x] 마우스 클릭
    - [x] 포커스/블러 관리
- [x] 검색어 유효성 검사
    - [x] 정규식 검증 (알파벳 1-6자)
    - [x] 실시간 에러 메시지
    - [x] 입력 제한
- [x] 검색 히스토리 삭제
    - [x] 개별 삭제 (✕ 버튼)
    - [x] 전체 삭제 버튼
    - [x] LocalStorage 동기화
- [x] 인기 종목 추천
    - [x] PopularStocks 컴포넌트
    - [x] 8개 인기 종목
    - [x] 클릭 이벤트 연동
    - [x] 반응형 그리드

### 4. 통합 및 최적화
- [x] App.js 리팩토링
- [x] 모든 기능 통합 테스트
- [x] 다크모드 전체 적용
- [x] 에러 처리 개선

---

## Phase 5: 차트 기능 확장 ✅

**목표**: 차트 인터랙션 및 분석 기능  
**진행률**: 100% ████████████████████  
**예상 진행률**: 80% → 90%

### 1. 기간 선택 기능 ✅
- [x] 버튼 UI 구현
    - [x] 7일 버튼
    - [x] 30일 버튼 (기본)
    - [x] 90일 버튼
    - [x] 1년 버튼
- [x] 선택된 기간 상태 관리
- [x] API 호출 days 파라미터 연동
- [x] 버튼 활성화 스타일
- [x] 로딩 상태 처리
- [x] 반응형 디자인 (모바일 2x2 그리드)
- [x] 다크모드 지원

### 2. 거래량 차트 ✅
- [x] 거래량 데이터 API 연동
    - [x] Backend StockHistoryDto volume 필드 추가
    - [x] YahooFinanceService volume 데이터 연동
- [x] 복합 차트 구현 (Chart.js)
    - [x] 가격 라인 차트
    - [x] 거래량 막대 차트
- [x] 이중 Y축 구성
    - [x] 왼쪽: 가격 ($)
    - [x] 오른쪽: 거래량 (K/M/B)
- [x] 상승일/하락일 색상 구분
    - [x] 상승일: 초록색 🟢
    - [x] 하락일: 빨간색 🔴
    - [x] 첫날: 파란색 🔵
- [x] 툴팁 표시 (가격 + 거래량)
- [x] 거래량 단위 자동 변환
- [x] 다크모드 지원

### 3. 이동평균선 (Moving Average) ✅
- [x] 이동평균선 계산 로직
    - [x] 5일 이동평균 (MA 5)
    - [x] 20일 이동평균 (MA 20)
    - [x] 60일 이동평균 (MA 60)
- [x] MovingAverageControl 컴포넌트
    - [x] 체크박스 UI
    - [x] 선택 상태 관리
- [x] 차트에 선 추가/제거
- [x] 범례 표시
- [x] 색상 구분 (핑크/보라/청록)
- [x] utils/movingAverage.js 유틸리티

### 4. 차트 비교 기능 ✅
- [x] CompareControl 컴포넌트
    - [x] 비교 모드 토글
    - [x] 종목 추가 입력
    - [x] 종목 제거 버튼
    - [x] 최대 5개 제한
- [x] 여러 종목 동시 표시
- [x] 정규화 퍼센트 비교
- [x] 종목별 색상 자동 할당
- [x] 범례 관리
- [x] 반응형 디자인

### 5. 차트 타입 선택 ✅
- [x] ChartTypeControl 컴포넌트
    - [x] 라인/캔들스틱 토글 버튼
    - [x] 아이콘 + 레이블 UI
- [x] 라인 차트 (기본)
- [x] 캔들스틱 차트
    - [x] chartjs-chart-financial 패키지
    - [x] OHLC 데이터 렌더링
    - [x] 상승/하락 색상 구분
    - [x] 거래량 차트 연동
    - [x] ⚠️ 시각화 개선 필요 (알려진 이슈)
- [x] 차트 타입별 기능 제한
    - [x] 캔들스틱: 이동평균선 비활성화
    - [x] 캔들스틱: 비교 모드 비활성화

### 6. 차트 확대/축소 ✅
- [x] chartjs-plugin-zoom 패키지 설치
- [x] ZoomControl 컴포넌트
    - [x] 리셋 버튼
    - [x] 사용 안내 메시지
- [x] 마우스 휠 줌 인/아웃
- [x] 드래그 패닝 (차트 이동)
- [x] 리셋 기능
- [x] 터치 제스처 지원 (pinch zoom)
- [x] 모든 차트 타입 적용
    - [x] 라인 차트
    - [x] 캔들스틱 차트
    - [x] 비교 차트

---

## Phase 6: 고급 기능 📅

**목표**: 추가 분석 및 관리 기능  
**진행률**: 0% ░░░░░░░░░░░░░░░░░░░░  
**예상 진행률**: 90% → 95%

### 1. 캔들스틱 개선
- [ ] chartjs-chart-financial 사용법 재검토
- [ ] 캔들스틱 렌더링 최적화
- [ ] 또는 OHLC 바 차트로 교체

### 2. 기술적 지표 (선택)
- [ ] RSI (Relative Strength Index)
- [ ] MACD
- [ ] 볼린저 밴드
- [ ] 지표 선택 UI

### 3. 관심 종목 (선택)
- [ ] LocalStorage 저장
- [ ] 관심 종목 목록
- [ ] 빠른 접근

### 4. 포트폴리오 (선택)
- [ ] 포트폴리오 페이지
- [ ] 종목 추가/제거
- [ ] 총 자산 계산
- [ ] 수익률 표시

---

## Phase 7: 배포 준비 📅

**목표**: 프로덕션 배포  
**진행률**: 0% ░░░░░░░░░░░░░░░░░░░░  
**예상 진행률**: 95% → 100%

### 1. 최적화
- [ ] React 빌드 최적화
- [ ] 이미지 최적화
- [ ] 코드 스플리팅
- [ ] Lazy Loading
- [ ] 번들 크기 분석

### 2. 테스트
- [ ] Unit Test (Jest)
- [ ] Integration Test
- [ ] E2E Test (Cypress)
- [ ] 성능 테스트
- [ ] 브라우저 호환성

### 3. Backend 배포
- [ ] Docker 이미지 생성
- [ ] AWS/Heroku 배포
- [ ] 환경변수 설정
- [ ] 헬스 체크 엔드포인트
- [ ] 로깅 설정

### 4. Frontend 배포
- [ ] Vercel/Netlify 배포
- [ ] 환경변수 설정
- [ ] CI/CD 파이프라인
- [ ] 도메인 연결
- [ ] HTTPS 설정

### 5. 모니터링
- [ ] 에러 트래킹 (Sentry)
- [ ] 분석 (Google Analytics)
- [ ] 성능 모니터링
- [ ] 로그 수집

---

## 🐛 버그 & 이슈

### 해결됨 ✅
- [x] Backend 429 Too Many Requests → Spring Cache 적용
- [x] 다크모드 깜빡임 → CSS Variables 사용
- [x] 로딩 상태 꼬임 → clearTimeout 적용
- [x] 검색 히스토리 삭제 불가 → 삭제 기능 추가
- [x] 자동완성 없음 → 자동완성 기능 추가
- [x] 차트 안 나옴 → data.data 구조 처리 추가
- [x] 거래량 데이터 없음 → Backend volume 필드 추가
- [x] 캔들스틱 date adapter 에러 → category scale 사용
- [x] 캔들스틱 색상 에러 → 각 데이터에 색상 직접 지정

### 진행 중 🔄
- 없음

### 알려진 이슈 ⚠️
- 캔들스틱 차트 시각화 개선 필요 (Phase 6에서 처리 예정)

---

## 📈 성능 지표

| 항목 | 목표 | 현재 | 상태 |
|------|------|------|------|
| 초기 로딩 시간 | < 2s | ~1.5s | ✅ |
| API 응답 시간 | < 500ms | ~300ms | ✅ |
| 번들 크기 | < 600KB | ~450KB | ✅ |
| Lighthouse 점수 | > 90 | 92 | ✅ |

---

## 📚 참고 자료

- [Yahoo Finance API](https://www.yahoofinanceapi.com/)
- [Chart.js Docs](https://www.chartjs.org/docs/latest/)
- [chartjs-plugin-zoom](https://www.chartjs.org/chartjs-plugin-zoom/)
- [chartjs-chart-financial](https://github.com/chartjs/chartjs-chart-financial)
- [React Router](https://reactrouter.com/)
- [Spring Boot](https://spring.io/projects/spring-boot)

---

## 🎯 다음 작업

**Phase 6: 고급 기능**
1. 캔들스틱 차트 개선
2. 기술적 지표 추가 (선택)
3. 관심 종목 기능 (선택)

**또는**

**Phase 7: 배포 준비**
1. 최적화 및 테스트
2. Backend/Frontend 배포
3. 모니터링 설정

---

## 📝 최근 업데이트 (v0.9.2)

### 2025-11-16
- ✅ **Phase 5 전체 완료**

#### Phase 5-A: 차트 타입 선택
- ✅ ChartTypeControl 컴포넌트 구현
- ✅ 라인 ↔ 캔들스틱 전환
- ✅ chartjs-chart-financial 패키지 연동
- ✅ 캔들스틱 OHLC 렌더링
- ✅ 기능 간 상호작용 (비교 모드 비활성화)

#### Phase 5-B: 차트 확대/축소
- ✅ chartjs-plugin-zoom 패키지 연동
- ✅ ZoomControl 컴포넌트 구현
- ✅ 마우스 휠 줌 기능
- ✅ 드래그 패닝 기능
- ✅ 리셋 버튼
- ✅ 터치 제스처 지원

- 📝 **문서 업데이트**
    - 루트 README.md 업데이트
    - CHECKLIST.md 업데이트
    - frontend/README.md 업데이트

---

### 2025-11-14
- ✅ **기간 선택 버튼** 구현 완료
    - 7일/30일/90일/1년 버튼
    - 선택 시 자동 재조회
    - 반응형 디자인

- ✅ **거래량 차트** 구현 완료
    - 복합 차트 (가격 + 거래량)
    - 이중 Y축
    - 상승/하락 색상 구분
    - Backend volume 필드 추가

- ✅ **이동평균선** 구현 완료
    - MA 5/20/60일 계산
    - 체크박스 UI
    - 차트 연동

- ✅ **차트 비교** 구현 완료
    - 최대 5개 종목 비교
    - 정규화 퍼센트 표시
    - 색상 자동 할당

---

**마지막 업데이트**: 2025-11-16  
**버전**: v0.9.2  
**진행률**: 90%